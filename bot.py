...
def create_callback(cmd_name, param_name):
    async def callback(interaction: discord.Interaction, **kwargs):
        # Access the dynamic parameter value from kwargs
...
async def sync_commands_from_json():
    ...
    for cmd_name, config in data.items():
        if cmd_name not in existing_cmds:
            # Extract the specific parameter name from the JSON (defaults to 'about')
            param_name = config.get("parameter_name", "about")

            # Create the dynamic command object
            ...
            # Manually inject the dynamic parameter into the command's internal params
            new_cmd._params = {
                param_name: app_commands.Parameter(
                    name=param_name,
                    description=f"Select {param_name}",
            ...
            # Register autocomplete to this specific parameter
            new_cmd.autocomplete(param_name)(create_autocomplete(cmd_name))
