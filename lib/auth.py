...
def is_authorized(user, permissions, owner_id):
    """Checks for wildcards (* or all) and IDs in blacklist/whitelist with audit logging."""
    if user.id == owner_id:
    ...
    if any(w in bl_users for w in wildcards) or any(w in bl_roles for w in wildcards):
        log_warn(f"Security: User {user} ({user.id}) hit Global Blacklist.")
        return False, "Global Blacklist Imposed"

    if user.id in bl_users or any(r_id in bl_roles for r_id in user_role_ids):
        log_warn(f"Security: User {user} ({user.id}) hit User/Role Blacklist.")
    ...
    log_warn(f"Auth: User {user} ({user.id}) denied access (Not on Whitelist).")
